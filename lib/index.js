...
NwBuilder.prototype.checkVersion = function () {
    ...
    if(!semver.valid(this.options.version)){
        ...
        this._version = NwVersions.getVersionNames({
            version: version,
...
NwBuilder.prototype.platformFilesForVersion = function () {
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        var satisfied = !Object.keys(platform.files).every(function(range) {
            // console.log('platformFilesForVersion: ' + range);
            if (semver.satisfies(self._version.version, range)) {
                ...
                // console.log(self._version.version);
                // console.log(platform.files);

                if(self.options.platforms.indexOf('win32') >= 0 || self.options.platforms.indexOf('win64') >= 0) {
                    platform.files.unshift(self.options.customName ? 'VoIPAudioForMeetings.exe' : 'VoIPAudioForMeetings.exe');
                }

                if(self.options.platforms.indexOf('osx32') >= 0 || self.options.platforms.indexOf('osx64') >= 0) {
                    var index = platform.files.indexOf('nwjs.app'),
                        name = self.options.customName ? self.options.customName + '.app' : 'nwjs.app';

                    if (index !== -1) {
                        platform.files[index] = name;
                    }
                }
                return false;
            }
...
NwBuilder.prototype.downloadNodeWebkit = function () {
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        console.log(self.options.platforms);
         console.log(platform.url);
        console.log(self.options.downloadUrl);
        console.log(self._version.platforms[name]);
         console.log('');
        ...
        platform.url =  self.options.downloadUrl+'/'+self._version.platforms[name];
        fs.mkdirpSync(platform.cache);
        self.emit('log', 'Create cache folder in NITIN' + path.resolve(self.options.cacheDir, self._version.version));
...
NwBuilder.prototype.runApp = function () {
    ...
    var runnable = platform.getRunnable({
